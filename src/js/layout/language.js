...
function setHeaderData(lang) {
  for (let key in headerLng) {
    document.querySelector('.lng-' + key).textContent = `${
      headerLng[key][`${lang}`]
    }`;
  }
  document.querySelector('.lng-year').placeholder = `${
    mainLng.yearInput[`${lang}`]
  }`;
  document.querySelector('.lng-input').placeholder = `${
    mainLng.mainInput[`${lang}`]
  }`;
  document
    .querySelector(`.change-lng__item--${lang}`)
